...
### 경로 표현식 용어 정리
- 상태 필드(statefield): 단순히 값을 저장하기 위한 필드(ex: m.username)
...
### 명시적 조인, 묵시적 조인
- 명시적 조인: join 키워드 직접 사용 (가능하면 명시적 조인을 사용하자)
...
- select t.members.uername from Team t -> 실패
- select m.uername from Team t join t.members m -> 성공

### 경로 탐색을 사용한 묵시적 조인 시 주의사항
- 항상 내부 조인
- 컬렉션은 경로 탐색의 끝, 명시적 조인을 통해 별칭을 얻어야함
- 경로 탐색능 주로 SELECT, WHERE 절에서 사용하지만 묵시적 조인으로 인해 SQL의 FROM (JOIN)절에 영향을 줌 
...
![image](https://user-images.githubusercontent.com/28394879/132968462-d81a1586-f5f6-44cb-b3fb-5f3d0809e298.png)
...
![image](https://user-images.githubusercontent.com/28394879/132969237-8d86b47f-760c-427d-a563-e876fa13c17a.png)
- 컬렉션 페치 조인을 했을떄 팀A에 대한 값이 2개나오기 떄문에 팀A가 중복으로 2번 등장하게 된다.
...
- SQL에 DISTINCT를 추가하지만 데이터가 다르므로 SQL 결과에서 중복제거 실패
  ![image](https://user-images.githubusercontent.com/28394879/132969338-51604542-98a7-40c5-936e-7b44d93c3132.png)
  
- DISTINCT가 추가로 애플리케이션에서 중복 제거시도
- 같은 식별자를 가진 Team 엔티티 제거
  ![image](https://user-images.githubusercontent.com/28394879/132969362-5e4d66d0-17eb-4fca-bfa8-6c0b09045447.png)

- 1:다 에서는 데이터 뻥튀기 ( TEAM으로 페치조인, 컬렉션 페치조인 ) / 다:1 에서는 데이터 뻥튀기 없음 ( MEMBER로 페치조인 )
...
- [JPQL] select t from Team t join jetch t.members where t.name = '팀A'
- [SQL] SELECT T.*, M.* FROM TEAM T INNER JOIN MEMBER M ON T.ID=M.TEAM_ID WHERE T.NAME='팀A'
...
- 둘 이상의 컬렉션은 페치 조인 할 수 없다.
    - 팀 안에 member도 있고 orders도 있으면 안됨.
